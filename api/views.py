...
class DetailQuotationView(APIView):
    ...
    def delete(self, request, pk, pk2):
        quotation = self._get_object(pk2)
...
class MarkView(APIView):
    ...
    def _get_models(self, mark_id):
        models = Model.objects.filter(mark_id=mark_id)
        return [{'id': model.id, 'name': model.name} for model in models]

    def get(self, request):
        marks = Mark.objects.all()
        marks_response = [{"id": mark.id, "name": mark.name, "models": self._get_models(mark_id=mark.id)} for mark in marks]
